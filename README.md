...
To open your database, simply use the `Open()` function:
...
```go
db, err := Open("my.db", 0600, &Options{Timeout: 1 * time.Second})
```
...
```go
db, err := Open("my.db", 0666, &Options{ReadOnly: true})
if err != nil {
	log.Fatal(err)
}
```
...
contain your database logic and a reference to a `*DB` with a initializing
constructor that takes in a filepath where the database file will be stored.
Neither Android nor iOS require extra permissions or cleanup from using this method.

```go
func NewBoltDB(filepath string) *BoltDB {
	db, err := Open(filepath+"/demo.db", 0600, nil)
	if err != nil {
		log.Fatal(err)
	}
...
* [BoltStore](https://github.com/yosssi/boltstore) - Session store using 
